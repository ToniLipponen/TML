...
namespace tml
{
    namespace Interface
    {
        ...
        template<ComponentAxis axis>
        void Slider<axis>::SetRoundness(float roundness)
        {
        ...
        template<ComponentAxis axis>
        void Slider<axis>::pDraw(RenderTarget& target)
        {
            ...
            if(axis == Horizontal)
            {
                ...
                target.DrawRect(m_pos + Vector2f(1, 1), Vector2f(x, (float)m_size.y) - Vector2f(2, 2), m_activeColor, m_roundness);
            }
            else
            {
