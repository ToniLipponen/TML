...
namespace tml
{
    ...
    void Sprite::Generate() noexcept
    {
        ...
        m_vertexData.emplace_back(Vertex{Math::Rotate(origin, m_pos, m_rotation),                           tl,           0x0, 0, Vertex::TEXTURE});
        m_vertexData.emplace_back(Vertex{Math::Rotate(origin, m_pos + Vector2f(m_size.x, 0.f), m_rotation), {br.x, tl.y}, 0x0, 0, Vertex::TEXTURE});
        m_vertexData.emplace_back(Vertex{Math::Rotate(origin, m_pos + Vector2f(0.f, m_size.y), m_rotation), {tl.x, br.y}, 0x0, 0, Vertex::TEXTURE});
        m_vertexData.emplace_back(Vertex{Math::Rotate(origin, m_pos + m_size, m_rotation),                  br,           0x0, 0, Vertex::TEXTURE});

        m_indexData.emplace_back(0);
        m_indexData.emplace_back(1);
        m_indexData.emplace_back(2);

        m_indexData.emplace_back(1);
        m_indexData.emplace_back(3);
        m_indexData.emplace_back(2);
    }
